...
function showModel(selectElement) {
  const selectedValue = selectElement.value;
  let brandModelsArr = mobiles[selectedValue];
  let modelNameArr = Object.keys(brandModelsArr);
  let model = document.createElement("select");
  (model.name = "model"), (model.id = "model");
  model.setAttribute("data-brand", selectedValue);
  model.onchange = function () {
    showModelDesc(this);
  };
  let disabledOption = document.createElement("option");
  disabledOption.disabled, disabledOption.selected;
  let disabledOptText = document.createTextNode("Select model name:");
  disabledOption.appendChild(disabledOptText);
  model.appendChild(disabledOption);

  for (let i in modelNameArr) {
    let option = document.createElement("option");
    let optionText = document.createTextNode(modelNameArr[i]);
    option.appendChild(optionText);
    option.value = modelNameArr[i];

    model.appendChild(option);
  }

  let ifModel = document.getElementById("model");
  ifModel ? ifModel.remove() : null;

  let divBrandModel = document.getElementById("brandModel");
  divBrandModel.appendChild(model);
}
...
function showModelDesc(desc) {
  const brand = desc.getAttribute("data-brand");
  const model = desc.value;
  ...
  for (let [key, value] of Object.entries(data)) {
    ...
    card.className = "card text-dark p-3 my-3";
    ...
    switch (key.toLowerCase()) {
      case "display":
        icon = `<i class="bi bi-phone" style="font-size: 1.2rem; color: #0d6efd;"></i>`;
        break;
      case "camera":
        icon = `<i class="bi bi-camera" style="font-size: 1.2rem; color: #dc3545;"></i>`;
        break;
      case "battery":
        icon = `<i class="bi bi-battery-full" style="font-size: 1.2rem; color: #198754;"></i>`;
        break;
      case "features":
        icon = `<i class="bi bi-stars" style="font-size: 1.2rem; color: #ffc107;"></i>`;
        break;
      default:
        icon = `<i class="bi bi-info-circle" style="font-size: 1.2rem; color: #6c757d;"></i>`;
    }

    let details = document.createElement("p");
    details.innerHTML = `<strong>${icon} ${capitalizeFirst(key)}</strong>`;

    if (typeof value === "object" && !Array.isArray(value)) {
      for (let [subKey, subValue] of Object.entries(value)) {
        if (typeof subValue === "object") {
          for (let [deepKey, deepValue] of Object.entries(subValue)) {
            details.innerHTML += `<br><span class="ms-3">🔹 <strong>${capitalizeFirst(subKey)}</strong> - <strong>${capitalizeFirst(deepKey)}</strong>: ${deepValue}</span>`;
          }
        } else {
          details.innerHTML += `<br><span class="ms-3">🔸 <strong>${capitalizeFirst(subKey)}</strong>: ${subValue}</span>`;
        }
      }
    } else if (Array.isArray(value)) {
      const capitalizedArray = value.map(item => capitalizeFirst(item));
      details.innerHTML += `<br><span class="ms-3">${capitalizedArray.join(", ")}</span>`;
    } else {
      details.innerHTML += `<br><span class="ms-3">${capitalizeFirst(value.toString())}</span>`;
    }

    card.appendChild(details);
    modelDetailsDiv.appendChild(card);
  }
}
